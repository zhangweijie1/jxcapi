 ...
 */
@Service
public class FinPayDailyOutServiceImpl {

    @Autowired
    private FinBillServiceImpl finBillService;
    @Autowired
    private FinPayDailyServiceImpl finPayDailyService;
    @Autowired
     ...
     * @return
     */
    public Result addDayilyOutData(FinBill bill) throws CustomsException {
        // 1. 单据
        bill.setDirection("0");
        ...
        if (bill.getComegoId() != null) {
            ...
            if (c != null) {
                // 是客户,应收1
                Customer customer = customerService.addDebt(bill.getComegoId(), thisDebt.negate());
    ...
    public Result getBillCode() throws CustomsException {
        return Result.ok(billOrderNoService.getBillCode(BillCatEnum.fin_dayily_out.getCode()));
    }
    ...
    public Result listDayilyOutData(PageParams<FinBill> pageParams) {
        Map<String, Object> data = pageParams.getData();
        data.put("billCat", BillCatEnum.fin_dayily_out.getCode());
        return Result.ok(finPayDailyService.listData(pageParams));
    }
}
